...
//整个kv server集群中的某个group下的1个server
type ShardKV struct {
	mu           sync.Mutex
	me           int
	rf           *raft.Raft //
	...
	gid          int
	masters      []*labrpc.ClientEnd //kv master for group/slot info
	maxraftstate int                 // snapshot if log grows this big

	kvs           [shardmaster.NShards]map[string]string //just for cur shard kvs
	...
	timer           *time.Timer //for update config from master server
	EnableDebugLog  bool
}
...
func (kv *ShardKV) putAppend(req *PutAppendArgs) Err {
	...
	if !kv.isRepeated(req.Me, req.MsgId) { //去重复
		...
		kv.println(&kv.rf, kv.gid, kv.me, "on", req.Op, req.Shard, "client", req.Me, "msgid:", req.MsgId, req.Key, ":", req.Value, kv.kvs[req.Shard][req.Key])
	}
	return OK
}
...
func (kv *ShardKV) onConfig(config *shardmaster.Config) {
	if config.Num > kv.nextConfig.Num {
		...
		kv.println(kv.gid, kv.me, config.Num, "update config", kv.nextConfig.Num, ":", shardmaster.GetShardsInfoString(&(kv.nextConfig.Shards)))
