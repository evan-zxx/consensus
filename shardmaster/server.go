...
type ShardMaster struct {
	...
	msgIDs   map[int64]int64 //消息去重
	killChan chan (bool)

	configs        []Config //indexed by config num 最新配置信息在最后面
	EnableDebugLog bool
}
...
func (sm *ShardMaster) join(args *JoinArgs) bool {
	sm.println(sm.me, "on join ", "msgid:", args.MsgId, GetGroupsInfoString(args.Servers))
	//消息去重
...
func (sm *ShardMaster) leave(args *LeaveArgs) bool {
	sm.println(args.Me, "on leave ", "msgid:", args.MsgId, GetSlientInfoString(args.GIDs))
	if sm.isRepeated(args.Me, args.MsgId, true) {
		return true
	}
	config := sm.getCurrentConfig()
...
func (sm *ShardMaster) move(args *MoveArgs) bool {
	...
	sm.appendConfig(&config)
	sm.println("on move ", "msgid:", args.MsgId, args.Shard, args.GID)
